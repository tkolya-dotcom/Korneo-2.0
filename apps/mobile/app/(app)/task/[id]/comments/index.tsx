...
export default function TaskCommentsScreen() {
  ...
  useEffect(() => {
    ...
    const channel = commentsApi.subscribe(id, 'task', () => {
      loadComments();
    });
  ...
  const loadComments = async () => {
    try {
      const data = await commentsApi.getByEntity(id, 'task');
      setComments(data || []);
    } catch (error) {
  ...
  const sendComment = async () => {
    ...
    setSending(true);
    try {
      await commentsApi.create(id, inputText.trim(), 'task');
      setInputText('');
      await loadComments();
    } catch (error) {
      console.error('Failed to send task comment:', error);
    } finally {
      setSending(false);
    }
  };
